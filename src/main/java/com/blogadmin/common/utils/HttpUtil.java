/*
 * Copyright 2015 gaoshou360.com All right reserved. This software is the
...
public class HttpUtil {
     ...
     * @return
     */
    public static String postRequest(String reqURL, Map<String, Object> params) {
        StringBuilder resultData = new StringBuilder();
        URL url = null;
        try {
            url = new URL(reqURL);
            ...
            try {
                ...
                if (params != null && params.size() > 0) {
                    for (Map.Entry<String, Object> entry : params.entrySet()) {
                        ...
                        sbd.append("=");
                        sbd.append(urlEncode(String.valueOf(entry.getValue())));
                        sbd.append("&");
                    }
                    sbd.setLength(sbd.length() - 1);// 删掉最后一个
                }
                String param = sbd.toString();
                urlConn.setRequestProperty("Content-Length", param.getBytes().length + "");//
                // urlConn.setRequestProperty("Connection", "Keep-Alive");
                // 设置长连接
                urlConn.connect();// 连接服务器发送消息
    ...
    public static void main(String[] args) {
        ...
        params.put("mobiles", "15058191683");
        params.put("content", "我是测试短信");
        params.put("appType", "TTID");
        params.put("sendType", "sms_code");
        String result = HttpUtil.postRequest("http://localhost:8098/dweb/sms_send_db", params);
        //String result = HttpUtil.postRequest(" http://139.129.40.205:8081/dweb_web/sms_send", params);
